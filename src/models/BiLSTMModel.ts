...
export class BiLSTMModel {
  ...
  private initializeWeights() {
    ...
    const denseWeights = Array(lstmUnits * 2).fill(0).map(() =>
    ...
    this.weights = {
      embedding,
      forwardLSTM: this.initializeLSTMWeights(embeddingDim, lstmUnits),
      backwardLSTM: this.initializeLSTMWeights(embeddingDim, lstmUnits),
      denseWeights,
      denseBias
    };
  }
  ...
  private lstmCell(input: number[], prevHidden: number[], prevCell: number[], weights: any): [number[], number[]] {
    const combined = [...input, ...prevHidden];

    const forgetGate = this.matMul(combined, weights.forgetGate).map(v => this.sigmoid(v));
    const inputGate = this.matMul(combined, weights.inputGate).map(v => this.sigmoid(v));
    const cellGate = this.matMul(combined, weights.cellGate).map(v => this.tanh(v));
    const outputGate = this.matMul(combined, weights.outputGate).map(v => this.sigmoid(v));
  ...
  private forward(sequence: number[]): number {
    ...
    let forwardHidden = Array(this.config.lstmUnits).fill(0);
    let forwardCell = Array(this.config.lstmUnits).fill(0);
    const forwardStates: number[][] = [];

    for (const input of embedded) {
      [forwardHidden, forwardCell] = this.lstmCell(input, forwardHidden, forwardCell, this.weights.forwardLSTM);
      forwardStates.push([...forwardHidden]);
    }

    let backwardHidden = Array(this.config.lstmUnits).fill(0);
    let backwardCell = Array(this.config.lstmUnits).fill(0);
    const backwardStates: number[][] = [];

    for (let i = embedded.length - 1; i >= 0; i--) {
      [backwardHidden, backwardCell] = this.lstmCell(embedded[i], backwardHidden, backwardCell, this.weights.backwardLSTM);
      backwardStates.unshift([...backwardHidden]);
    }

    const combinedHidden = [...forwardHidden, ...backwardHidden];

    const output = this.matMul(combinedHidden, this.weights.denseWeights);
  ...
  train(data: TrainingData, epochs: number = 10, learningRate: number = 0.001): void {
    ...
    for (let epoch = 0; epoch < epochs; epoch++) {
      ...
      console.log(`BiLSTM Epoch ${epoch + 1}/${epochs}, Loss: ${(totalLoss / data.sequences.length).toFixed(4)}`);
    }
  }
